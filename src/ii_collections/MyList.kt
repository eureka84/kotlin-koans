...
interface MyList<T> {
    ...
    fun <R> foldLeft(initial: R, combine: (acc: R, elem: T) -> R): R
}
...
object Nil: MyList<Any> {
    ...
    override fun <R> foldLeft(initial: R, combine: (acc: R, elem: Any) -> R): R = initial
}
...
data class Cons<T>(private val head: T, private val tail: MyList<T>): MyList<T> {
    ...
    override fun <R> foldLeft(initial: R, combine: (acc: R, elem: T) -> R): R {
